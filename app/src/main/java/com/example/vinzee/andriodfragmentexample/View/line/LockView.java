package com.example.vinzee.andriodfragmentexample.View.line;
...
//      rowCount = typedArray.getInteger(R.styleable.LockView_rowCount, DEFAULT_ROW_COUNT);
    ...
    private void init() {
        ...
        innerCirclePaint.setTextSize(100);
...
//            n = Integer.parseInt(list.get(i).toString());
            n = 1+ rand.nextInt(6)+rand.nextInt(3)+rand.nextInt(2);
            points[i].set((i % rowCount * 3 + m) * radius, (i / rowCount * 3 + n) * radius);
        }
    }
    ...
    private void drawCircle(Canvas canvas) {
        ...
        for (int index = 0; index < rowCount * rowCount; index++) {
            ...
            canvas.drawText(Integer.toString(index),points[index].x-30,points[index].y+30,innerCirclePaint);
        }
    }
